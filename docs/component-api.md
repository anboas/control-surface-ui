...
| Schedule Gantt | `.if-schedule-gantt` | panel, header, controls, scroller, rows, bars, period rows, pagination, card grid, legend | Framework-owned schedule/period planning surface. Apps provide row data and dynamic bar positions only. |
